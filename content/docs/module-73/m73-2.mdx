---
title: "Protecting Secrets with Environment Variables"
description: "Lesson 73.2: Protecting Secrets with Environment Variables."
---

We should use `.env` to store our database connection URL. This is how we can do it in Go:
...
Make sure to match the environment variable's name is the exact same in both the code file and the env file,
which is `DATABASE_URL` in this case.

Also, definitely add the env file in the `.gitignore` file.
