---
title: "Refactoring and Building the Project"
description: "Lesson 73.3: Refactoring and Building the Project."
---

We can separate the code into different files for a cleaner code.

- `main.go` for the main code
- `models.go` for the schema structs
- `db.go` to store the DB connection
- `handlers.go` for the handlers
...
We're using a single package (`main`) for simplicity, but we can use multiple packages too.
We just have to add imports for those and use capital letters for the function names.
